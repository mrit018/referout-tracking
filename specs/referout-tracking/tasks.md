...
- [ ] T006 [INF] Add `src/utils/referoutSql.ts` with builders: `buildKpiSql`, `buildEmergencyDistSql`, `buildTypeAreaSql`, `buildCauseTopSql`, `buildTrendSql`, `buildDetailSql`, `buildDetailByIdSql`, `buildTimelineSql`, `buildSqlParams(filters, dbType)`. Each returns `{sql, params}` and **must** start with SELECT or WITH. Test first.
...
### Tests
- [ ] T010 [P][US1] Component test: `tests/component/KpiCardsGrid.test.tsx` — given fixed kpis prop, renders 7 cards with correct numbers, deltas, colors.

### Implementation
- [ ] T011 [US1] Add `src/components/referout/KpiCardsGrid.tsx` with 7 cards (4 geography + 3 operational). Uses shadcn `<Card>`, lucide icons (Users, MapPin, MapPinned, Globe, Siren, Ambulance, HeartPulse), and `formatPercentDelta`. Click handler prop forwards to KPI drill-down dialog.
...
- [ ] T077 [INF] Manual verification checklist (in `tests/manual-checklist.md`):
  ...
  - [ ] All 7 KPI cards render with deltas
  - [ ] All 4 charts render
...
- T010, T020, T021, T030, T040, T050, T060 can run in parallel (different files)
