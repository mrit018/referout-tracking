...
### User Story 7 — Refer Back Tracking (Priority: P2)

เจ้าหน้าที่งานส่งต่อต้องการติดตามการรับผู้ป่วยกลับ (Refer Back) เพื่อดูภาพรวมการรับส่งต่อทั้งขาออกและขาเข้า

**Why this priority**: ข้อมูลจากผู้ใช้ระบุ "Refer Back" เป็นส่วนที่ 4 ของ dashboard คู่กับ Refer Out ทั้ง 3 พื้นที่.

**Independent Test**: เลือกช่วงวันที่ที่มีข้อมูล referin ตรวจว่าตาราง Refer Back แสดงข้อมูลถูกต้องพร้อมสาขา SERVICE PLAN, ICD 10, โรงพยาบาล, สาเหตุ.

**Acceptance Scenarios**:

1. **Given** มีข้อมูล referin 50 รายการในช่วง, **When** เปิดแท็บ/ส่วน Refer Back, **Then** แสดงตารางพร้อม: referin_number, refer_date, HN, ชื่อผู้ป่วย, ICD 10, รพ.ต้นทาง, รพ.ปลายทาง, สาเหตุการส่งต่อ
2. **Given** มีข้อมูล referin ในหลายสาขา SERVICE PLAN, **When** ดูสรุป, **Then** แสดงจำนวนแยกตามสาขา SERVICE PLAN (ผ่าน `spclty` field)
3. **Given** ผู้ใช้คลิกแถว, **When** คลิก, **Then** เปิด Dialog แสดงรายละเอียดเต็มของการ refer back
4. **Given** ผู้ใช้ส่งออก CSV, **When** คลิก, **Then** ดาวน์โหลดไฟล์ `referback-yyyy-MM-dd_HH-mm.csv` ที่เปิดใน Excel ได้ถูกต้อง

---

### User Story 8 — SERVICE PLAN Analytics (Priority: P2)

ผู้บริหารต้องการเห็นการส่งต่อแยกตามสาขา SERVICE PLAN (STEMI, Stroke, Trauma, Cancer, Sepsis, Pregnancy, New born, อื่นๆ) เพื่อวางแผนพัฒนาศักยภาพ

**Why this priority**: ระบุโดยตรงในข้อมูลที่ผู้ใช้ให้มา — "สาขา SERVICE PLAN" เป็นมิติหลักของทุกส่วนของ dashboard.

**Independent Test**: เลือกช่วงข้อมูล ตรวจว่า SERVICE PLAN breakdown รวมกันเท่ากับ total.

**Acceptance Scenarios**:

1. **Given** มีข้อมูลใน 5 สาขา SERVICE PLAN (STEMI=5, Stroke=10, Trauma=15, Cancer=8, อื่นๆ=12), **When** ดูสรุป, **Then** แสดงทั้ง 5 สาขาพร้อมจำนวน
2. **Given** ผู้ใช้เลือก SERVICE PLAN "Stroke", **When** เลือก, **Then** กรองข้อมูลเฉพาะ Stroke ทั้ง KPI/กราฟ/ตาราง
3. **Given** SERVICE PLAN breakdown ในแต่ละพื้นที่ (ในจังหวัด/ในเขต/นอกเขต), **When** ดู, **Then** แสดงจำนวนแยกตาม SERVICE PLAN ในแต่ละพื้นที่
4. **Given** ผู้ใช้คลิกสาขา SERVICE PLAN ใดๆ, **When** คลิก, **Then** drill-down แสดงรายชื่อผู้ป่วยในสาขานั้น

---

### User Story 9 — ICD-10 Diagnosis Analytics (Priority: P2)

ผู้บริหารต้องการเห็นรหัสโรค ICD 10 ที่เป็นสาเหตุหลักของการส่งต่อ เพื่อจัด_priority พัฒนาศักยภาพ

**Why this priority**: ระบุโดยตรงในข้อมูลที่ผู้ใช้ให้มา — "รหัสโรค ICD 10" เป็นมิติหลักของทุกส่วน.

**Independent Test**: เลือกช่วงข้อมูล ตรวจว่า ICD 10 top-N แสดงถูกต้องและรวมได้.

**Acceptance Scenarios**:

1. **Given** มี pdx หลากหลาย, **When** ดูสรุป, **Then** แสดง top-10 ICD 10 พร้อมจำนวนและคำอธิบายโรค
2. **Given** ผู้ใช้เลือก ICD 10 ใดๆ, **When** เลือก, **Then** กรองเฉพาะรายการที่มี pdx นั้น
3. **Given** ดู ICD 10 ในแต่ละพื้นที่, **When** เลือกพื้นที่, **Then** แสดง ICD 10 top-10 ของพื้นที่นั้น
4. **Given** ผู้ใช้คลิก ICD code, **When** คลิก, **Then** drill-down แสดงรายชื่อผู้ป่วยที่มี pdx นั้น
...
- **FR-002**: ระบบต้องแสดง KPI cards 7 ใบ: ทั้งหมด, ในจังหวัด, ในเขต, นอกเขต, คุกคามชีวิต, ใช้รถพยาบาล, มีพยาบาลร่วม
- **FR-003**: ระบบต้องคำนวณพื้นที่ 3 ระดับ: "ในจังหวัด" จาก `hospcode.chwpart = '71'` (กาญจนบุรี), "ในเขต" จาก `hospcode.region_id = 5` ยกเว้น `hospcode IN (' банзвестной', '...')` (รพ.บ้านแพ้ว, รพ.วัดไร่ขิง), "นอกเขต" จากที่เหลือทั้งหมด; ใช้ `refer_in_province`/`refer_in_region` เป็น primary signal และ `hospcode.chwpart`/`region_id` เป็น secondary validation
...
- **FR-008**: ระบบต้องรองรับการกรอง: ช่วงวันที่ (preset + custom), `referout_type_id`, `referout_status_id`, `referout_emergency_type_id`, `referout_sp_type_id` (SERVICE PLAN), area bucket (ในจังหวัด/ในเขต/นอกเขต), `pdx` (ICD 10), free-text search
- **FR-009**: ระบบต้องแสดงกราฟ 5 ตัว: donut ระดับเร่งด่วน, stacked bar type×area, horizontal bar top-10 refer_cause, **horizontal bar SERVICE PLAN breakdown**, line trend จำนวนต่อวัน/เดือน
...
- **FR-019**: ระบบต้อง LEFT JOIN lookup tables (`hospcode`, `referout_type`, `referout_emergency_type`, `referout_status`, `refer_cause`, `referout_sp_type`, `patient`) เพื่อให้ลูปขาดยังคงแสดงข้อมูลหลัก
...
- **FR-021**: ระบบต้องแสดงสาขา SERVICE PLAN (จาก `referout_sp_type_id` → `referout_sp_type`) เป็นมิติหลัก: STEMI, Stroke, Trauma, Cancer, Sepsis, Pregnancy/Labor/Postpartum, New born, อื่นๆ — แสดงในทุกส่วนของ dashboard (KPI breakdown, กราฟ, ตาราง, drill-down)
- **FR-022**: ระบบต้องแสดงรหัสโรค ICD 10 (จาก `pdx`) เป็นมิติวิเคราะห์: top-10 รหัสโรค, กรองตาม ICD, drill-down เห็นรายชื่อผู้ป่วยตามรหัส
- **FR-023**: ระบบต้องแสดงสาเหตุการส่งต่อ 5 ประเภท: (1) เพื่อการวินิจฉัยรักษา, (2) เพื่อการวินิจฉัย, (3) เพื่อการรักษาต่อเนื่อง, (4) ส่งต่อตามสิทธิ์, (5) ญาติและผู้ป่วยต้องการ — แมปจาก `refer_cause` field หรือ hardcode mapping ถ้า lookup ไม่ตรง
- **FR-024**: ระบบต้องแสดงส่วน "Refer Back" แยกต่างหาก — ดึงข้อมูลจากตาราง `referin` แสดง: สาขา SERVICE PLAN, จำนวนคน/ครั้ง, รหัสโรค ICD 10, รพ.ต้นทาง/ปลายทาง, สาเหตุการส่งต่อ 5 ข้อ — มีตาราง, สรุป, และ CSV export เช่นเดียวกับ Refer Out
- **FR-025**: Dashboard ต้องจัดเป็น 4 ส่วนหลัก: (1) Refer Out ในจังหวัด, (2) Refer Out ในเขต, (3) Refer Out นอกเขต, (4) Refer Back — แต่ละส่วนแสดงสาขา SERVICE PLAN, จำนวน, ICD 10, โรงพยาบาล, สาเหตุการส่งต่อ

### Key Entities

- **Referral (referout)**: เรคคอร์ดใบส่งตัวออก 1 ใบ. คีย์: `referout_id`. ฟิลด์สำคัญ: refer_number, refer_date/time, hn, vn, hospcode, pre_diagnosis, pdx, refer_in_province, refer_in_region, referout_type_id, referout_emergency_type_id, referout_status_id, referout_sp_type_id, refer_cause, with_nurse, with_ambulance, doctor, depcode, hpi, pmh, lab_text, treatment_text, request_text, ptstatus_text, refer_response_type_id, refer_response_text
- **DestinationHospital (hospcode)**: รพ.ปลายทาง. คีย์: `hospcode`. ฟิลด์: name, hosptype, chwpart (จังหวัด), region_id (เขตสุขภาวะ), area_code.
...
- **ServicePlanType (referout_sp_type)**: 8 สาขา — 1 STEMI, 2 Stroke, 3 Trauma, 4 Cancer, 5 Sepsis, 6 Pregnancy/Labor/Postpartum, 7 New born, 99 อื่นๆ. เชื่อมผ่าน `referout.referout_sp_type_id`.
- **ReferralCause (refer_cause)**: สาเหตุการส่งต่อ 5 ประเภท: (1) เพื่อการวินิจฉัยรักษา, (2) เพื่อการวินิจฉัย, (3) เพื่อการรักษาต่อเนื่อง, (4) ส่งต่อตามสิทธิ์, (5) ญาติและผู้ป่วยต้องการ.
...
- **ReferBack (referin)**: เรคคอร์ดการรับผู้ป่วยกลับ 1 ราย. คีย์: `vn`. ฟิลด์สำคัญ: refer_date, refer_time, hn, hospcode (รพ.ต้นทาง), refer_hospcode (รพ.ปลายทาง), icd10, pre_diagnosis, refer_type, referin_number, referout_number, f43_causein_id.
...
- **SC-003**: ผลรวม "ในจังหวัด + ในเขต + นอกเขต" ต้องเท่ากับ "ทั้งหมด" ในทุกช่วงวันที่ (data integrity test)
...
12. **Geography 3-bucket classification** — "ในจังหวัด" = `hospcode.chwpart = '71'` (กาญจนบุรี), "ในเขต" = `hospcode.region_id = 5` ยกเว้น รพ.บ้านแพ้ว (`hospcode` TBD) และ รพ.วัดไร่ขิง (`hospcode` TBD), "นอกเขต" = ที่เหลือทั้งหมด. ใช้ `refer_in_province`/`refer_in_region` เป็น primary signal, `hospcode.chwpart`/`region_id` เป็น secondary validation. ถ้าทั้งคู่ NULL → ใส่ bucket "นอกเขต" เป็น default.
...
18. **Province code 71** = กาญจนบุรี (FIPS/MOPH standard).
...
21. **SERVICE PLAN mapping** — `referout_sp_type_id` → `referout_sp_type` table. 8 branches: STEMI, Stroke, Trauma, Cancer, Sepsis, Pregnancy/Labor/Postpartum, New born, อื่นๆ. NULL → แสดง "ไม่ระบุสาขา".
22. **5 Referral causes mapping** — สาเหตุการส่งต่อ 5 ข้อ: (1) เพื่อการวินิจฉัยรักษา, (2) เพื่อการวินิจฉัย, (3) เพื่อการรักษาต่อเนื่อง, (4) ส่งต่อตามสิทธิ์, (5) ญาติและผู้ป่วยต้องการ — map จาก `refer_cause` field; ถ้า lookup ไม่ตรงให้ hardcode mapping.
23. **Refer Back scope** — ข้อมูลจากตาราง `referin` ทั้งหมดใน session scope (โรงพยาบาลเดียว). มิติข้อมูลเช่นเดียวกับ Refer Out: SERVICE PLAN (ผ่าน `spclty` field), ICD 10 (`icd10` field), โรงพยาบาล, สาเหตุ.
24. **Dashboard 4-section layout** — แสดง 4 ส่วนหลัก: Refer Out ในจังหวัด, Refer Out ในเขต, Refer Out นอกเขต, Refer Back. แต่ละส่วนมี sub-view: สาขา SERVICE PLAN, จำนวน, ICD 10, โรงพยาบาล, สาเหตุ.
